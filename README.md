...
- 🎯 Entry points to execute your program using `python -m <aforismi>` or `$ aforismi` with basic CLI argument parsing.
...
[![codecov](https://codecov.io/gh/revego/aforismi/branch/main/graph/badge.svg?token=aforismi_token_here)](https://codecov.io/gh/revego/aforismi)
[![CI](https://github.com/revego/aforismi/actions/workflows/main.yml/badge.svg)](https://github.com/revego/aforismi/actions/workflows/main.yml)

Awesome aforismi created by revego

## Install

from source
```bash
git clone https://github.com/revego/aforismi aforismi
cd aforismi
make install
```
...
```bash
pip install aforismi
```

## Executing

```bash
$ aforismi run --port 8080
```

or

```bash
python -m aforismi run --port 8080
```

or

```bash
$ uvicorn aforismi:app
```
...
```bash
❯ aforismi --help
Usage: aforismi [OPTIONS] COMMAND [ARGS]...
...
```bash
❯ aforismi create-user --help
Usage: aforismi create-user [OPTIONS] USERNAME PASSWORD
...
```bash
aforismi create-user admin admin --superuser
```
...
Run with `aforismi run` and access http://127.0.0.1:8000/docs

![](https://raw.githubusercontent.com/rochacbruno/fastapi-project-template/master/docs/api.png)


**For some api calls you must authenticate** using the user created with `aforismi create-user`.
...
-----------------------------------------------------
aforismi/__init__.py              4      0   100%
aforismi/app.py                  16      1    94%
aforismi/cli.py                  21      0   100%
aforismi/config.py                5      0   100%
aforismi/db.py                   10      0   100%
aforismi/models/__init__.py       0      0   100%
aforismi/models/content.py       47      1    98%
aforismi/routes/__init__.py      11      0   100%
aforismi/routes/content.py       52     25    52%
aforismi/routes/security.py      15      1    93%
aforismi/routes/user.py          52     26    50%
aforismi/security.py            103     12    88%
-----------------------------------------------------
TOTAL                               336     66    80%
...
```py
from aforismi.config import settings
```
...
> `dynaconf_merge` is a boolean that tells if the settings should be merged with the default settings defined in aforismi/default.toml.

### As environment variables
```bash
export aforismi_KEY=value
export aforismi_KEY="@int 42"
export aforismi_KEY="@jinja {{ this.db.uri }}"
export aforismi_DB__uri="@jinja {{ this.db.uri | replace('db', 'data') }}"
```
...
```bash
aforismi_ENV=production aforismi run
```
